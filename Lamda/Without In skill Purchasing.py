...
def amazon_help():
    ...
    card_content = "HELP"
    speech_output = "<speak>"\
                    "Anytime in this mission, You can say repeat, get hints, or check my bag. "\
                    "repeat, to repeat the content, "\
                    "get hints, to get some clues, "\
                    "or check my bag to know the things you have. "\
                    "you can also say explore, and use on the things you find. "\
                    "</speak>"
    reprompt_text = "You can say get hints. "
...
def get_welcome_response():
    ...
    speech_output =  "<speak>"\
                     ...
                     "what would you do? "\
                     "in this mission, you can say repeat, get hints, or check my bag. "\
                     "</speak>"
    reprompt_text =  "start playing by saying, get hint or repeat. "
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
        card_title, card_content , speech_output, reprompt_text, should_end_session))
...
def load_game():
    if(data.load_game==0):
        ...
        speech_output = "<speak>"\
                        ...
                        "The intelligence have sent you the mission's protocol.<break time='1s'/> "\
...
def help_me():
    ...
    if(data.help_no==1):
        ...
        speech_output = "<speak>"\
                        ...
                        "whenever you are in need of hints, just say get hint. "\
        ...
        data.repeat_no=2
        speech_output = "<speak><voice name='Brian'><lang xml:lang='en-GB'>Think something and escape commander. </lang></voice></speak>"
...
def repeat():
    if(data.repeat_no==0):
        ...
        return build_response(session_attributes, build_speechlet_response(
            card_title, card_content, speech_output, reprompt_text, should_end_session))
    elif(data.repeat_no==51):
        return amazon_help()
    elif(data.repeat_no==52):
        return explore_auto()
    elif(data.repeat_no==53):
        return explore_iphone()
...
def use_mag():
    ...
    if(data.mag_no==0):
        ...
        speech_output = "<speak>you can use it.</speak>"
    elif(data.mag_no==2):
        data.help_no=29
...
def closetlock(intent, session):
    card_title = "STATE TREASURE"
    card_content = "Two digit lock"
    session_attributes = {}
    should_end_session = False
    solnti = intent['slots']['two_digit']['value']
    if(solnti=='46' or solnti == '460'):
        data.repeat_no=41
        data.help_no=25
        data.verify_no=4
        data.load_game=3
...
def elelock(intent, session):
    data.key_no=2
    card_title = "STATE TREASURE"
    card_content = "Elevator Lock"
    session_attributes = {}
    should_end_session = False
    soln = intent['slots']['three_digits']['value']
    if(soln=='735' or soln == '7350'):
        data.load_game=1
        data.repeat_no=31
        data.help_no=16
        data.lift_no=0
...
def explore_auto():
    data.repeat_no=52
    card_title = "STATE TREASURE"
    card_content = "Explore Auto..!!"
    session_attributes = {}
    should_end_session = False
    speech_output = "<speak>"\
                    "It is a normal yellow auto, with the numbers 2, 0, 0, 1. </speak> "
    reprompt_text = "These are useless clues. "
    return build_response(session_attributes, build_speechlet_response(
            card_title, card_content, speech_output, reprompt_text, should_end_session)) 
...
def explore_iphone():
    data.repeat_no=53
    card_title = "STATE TREASURE"
    card_content = "Explore Iphone..!!"
    session_attributes = {}
    should_end_session = False
    speech_output = "<speak>"\
                    "It is a brand new apple iphone ten S.</speak> "
    reprompt_text = "These are useless clues. "
    return build_response(session_attributes, build_speechlet_response(
            card_title, card_content, speech_output, reprompt_text, should_end_session))             
...
def invalid():
    ...
    session_attributes = {}
    should_end_session = False
    speech_output = "<speak>sorry i could not recognize that. you can say get hint.</speak>"
    reprompt_text = "say get hint. "
    return build_response(session_attributes, build_speechlet_response(
        card_title, card_content, speech_output, reprompt_text, should_end_session))
...
def on_intent(intent_request, session):
    ...
    if intent_name == "HintIntent":
        ...
        return wall_paint()
    elif intent_name == "AutoIntent":
        return explore_auto()
    elif intent_name == "IphoneIntent":
        return explore_iphone()    
    elif intent_name == "ExploreRoomLock":
        return explore_room_lock()
    elif intent_name == "EleLock":
        return elelock(intent, session)     
    elif intent_name == "ClosetLock":
        return closetlock(intent, session)
